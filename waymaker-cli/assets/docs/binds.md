...
Get your key name with `wm --test-keys`.
...
When sharing a command-line waymaker command, you can also define your actions using these semantic triggers, allowing consumers to use their preferred binds for similar actions across different applications.
...
| `Sort(order)`       | Sort results by specified order (`Alphabetical`, `AlphabeticalReverse`, `Natural`, `NaturalReverse`, `Modified`, `ModifiedReverse`, `Created` / `btime`, `CreatedReverse`, `Size`, `SizeReverse`, `Extension`, `ExtensionReverse`, or reset if empty). |
...
| Action                 | Description                                                                                                                         |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `Execute(cmd)`         | Run a shell command interactively with full TTY (`stdin`, `stdout`, `stderr` connected to `/dev/tty`), pause TUI, and return to waymaker on exit. Also records frecency for targeted items. |
...
| `BecomeSilent(cmd)`    | Transform the process into the command without clearing the screen (useful for transitioning between different waymaker presets). |
...
wm b 'alt-enter=Accept'

# Bind multiple actions to one key:
wm b.ctrl-s='Select|||Down'

# Some action parameters are optional
wm b.ctrl-q 'SwitchPreview'
```
