...
Waymaker (`wm`) includes a built-in, high-performance **Frecency** (Frequency + Recency) tracking engine designed to track, rank, and provide instant access to your most frequently and recently accessed files and directories.
...
- **Database Path**: Stored in the user's state directory:
  - Linux/macOS: `~/.local/state/waymaker/frecency.redb`
  - Windows: `%LOCALAPPDATA%\waymaker\frecency.redb`
...
When `frecency = true` (or `--frecency` / `-f`) is enabled (such as in **Jump Mode** `wm -o jump`, the default config, or shell integrations `j` / `z`):
...
### Record & Query
- **`wm add <path>`**: Records an access event for a file or directory path (increases score and timestamp).
- **`wm rank <path>`**: Displays the current frecency score, total access count, last access time, and raw records for a path.
- **`wm list [-d / --dirs / --dirs-only] [keywords...]`** / **`wm query`**: Lists tracked paths sorted by frecency score descending, filtered to items matching ALL specified keywords.
  - Option `-d` / `--dirs`: Filters and outputs **directories only** (used in `jump` shell integrations like `j` / `z`).

### Maintenance & Management
- **`wm rm <path>`** / **`wm remove <path>`**: Manually deletes a path entry from the frecency database.
- **`wm clean`** / **`wm prune`**: Scans the database and purges stale entries (paths that no longer exist on the local filesystem).
- **`wm cache [path]`**: Warm-indexes directory structures into local cache for ultra-fast startup.

### Integrations & Migrations
- **`wm import zoxide`**: Imports historical directory access records and scores directly from `zoxide`.
- **`wm init <shell> [--cmd <alias>]`**: Generates shell integration code for `zsh`, `bash`, `fish`, `nushell`, or `powershell` with optional command alias (e.g. `wm init zsh --cmd j`).
...
Unlike filesystem-only indexers (such as FFF daemon) that only index directory paths, Waymaker's `frecency.rs` engine operates on **arbitrary string keys**. When piping text lines from any shell command into `wm` (`command | wm`), selection events automatically record frecency scores for those exact string lines.

### Key Use Cases:

- **Docker Containers & Kubernetes Pods**: `docker exec -it $(docker ps --format '{{.Names}}' | wm) bash`  
...
- **Git Branches**: `git checkout $(git branch --sort=-committerdate | awk '{print $1}' | wm)`  
  Active feature branches stay prioritized at the top of the list during branch switching.
- **SSH Hosts**: `ssh $(grep -i "^Host " ~/.ssh/config | awk '{print $2}' | grep -v '*' | wm)`  
  Frequently visited SSH remote hosts are automatically boosted in search results.
- **Tmux Sessions**: `tmux switch-client -t $(tmux list-sessions -F "#{session_name}" | wm)`  
