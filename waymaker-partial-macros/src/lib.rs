...
pub fn partial(attr: TokenStream, item: TokenStream) -> TokenStream {
        ...
        if let Some((kind, inners)) = coll_info {
                                ...
                                quote! {
                                    ...
                                    for target in #target_expr.iter_mut() {
                                        if let Some(p_item) = p_it.next() {
                                            waymaker_partial::Apply::apply(target, p_item);
                                        } else {
                                            break;
                                        }
                                    }
                                    for p_item in p_it {
                                        ...
                                        waymaker_partial::Apply::apply(&mut t, p_item);
                                        #target_expr.push(t);
                                    }
                                }
                            } else {
                            ...
                            quote! {
                                for (k, p_v) in p {
                                    if let Some(v) = #target_expr.get_mut(&k) {
                                        waymaker_partial::Apply::apply(v, p_v);
                                    } else {
                                        let mut v = <#element_ty as Default>::default();
                                        waymaker_partial::Apply::apply(&mut v, p_v);
            ...
            if let Some(field_ident) = &field.ident {
                ...
                let set_logic = if is_sequence {
                    ...
                    quote! {
                        let deserialized: #partial_coll_ty = waymaker_partial::deserialize(val)?;
                        #assignment
                    }
                } else {
                    ...
                    if inners.len() == 2 {
                        ...
                        let descent_logic = if should_recurse || is_set_recurse {
                            let set_item_logic = if should_recurse {
                                quote! { waymaker_partial::Set::set(item, rest, val)?; }
                            } else {
                                quote! {
                                    let mut p_item = #p_element_ty::default();
                                    waymaker_partial::Set::set(&mut p_item, rest, val)?;
                                    *item = waymaker_partial::from(p_item);
                                }
                            };
                            ...
                            quote! {
                                if rest.is_empty() {
                                    ...
                                    let (key, value): (#key_ty, #val_ty) = waymaker_partial::deserialize(&combined)?;
                                    let _ = #target.insert(key, value);
                                } else {
                                    let key: #key_ty = waymaker_partial::deserialize(&[key_str.clone()])?;
                            ...
                            quote! {
                                if rest.is_empty() {
                                    let key: #key_ty = waymaker_partial::deserialize(&[key_str.clone()])?;
                                    let value: #val_ty = waymaker_partial::deserialize(&val)?;
                                    let _ = #target.insert(key, value);
                                } else {
                                    return Err(waymaker_partial::PartialSetError::ExtraPaths(rest.to_vec()));
                                }
                            }
                        };
                        ...
                        quote! {
                            if let Some((key_str, rest)) = tail.split_first() {
                                ...
                                let new_map: #partial_coll_ty = waymaker_partial::deserialize(val)?;
                                #set_full_coll_logic
                            }
                        }
                    } else {
                        ...
                        if is_set_recurse {
                            if should_recurse {
                                quote! {
                                    ...
                                    if tail.is_empty() {
                                        item = waymaker_partial::deserialize(val)?;
                                    } else {
                                        waymaker_partial::Set::set(&mut item, tail, val)?;
                                    }
                                ...
                                quote! {
                                    if tail.is_empty() {
                                        let item: #item_ty = waymaker_partial::deserialize(val)?;
                                        #target.#push_method(item);
                                    } else {
                                        let mut p_item = #p_element_ty::default();
                                        waymaker_partial::Set::set(&mut p_item, tail, val)?;
                                        let item: #item_ty = waymaker_partial::from(p_item);
                                        #target.#push_method(item);
                                    }
                                }
                            }
                            ...
                            quote! {
                                if let Some((_, _)) = tail.split_first() {
                                    return Err(waymaker_partial::PartialSetError::ExtraPaths(tail.to_vec()));
                                }
                                let item: #item_ty = waymaker_partial::deserialize(val)?;
                                #target.#push_method(item);
                            }
            ...
            if is_recursive_field {
                if !field_unwrap && is_opt {
                    apply_field_stmts.push(quote! {
                        if let Some(p) = partial.#field_name {
                            if let Some(ref mut v) = self.#field_name {
                                waymaker_partial::Apply::apply(v, p);
                            } else {
                                self.#field_name = Some(waymaker_partial::from(p));
                            }
                        }
                    });
                    merge_field_stmts.push(quote! {
                        match (&mut self.#field_name, other.#field_name) {
                            (Some(s), Some(o)) => waymaker_partial::Merge::merge(s, o),
                    ...
                    apply_field_stmts.push(quote! {
                        if let Some(ref mut v) = self.#field_name {
                            waymaker_partial::Apply::apply(v, partial.#field_name);
                        } else {
                            self.#field_name = Some(waymaker_partial::from(partial.#field_name));
                        }
                    });
                    merge_field_stmts.push(quote! { waymaker_partial::Merge::merge(&mut self.#field_name, other.#field_name); });
                    clear_field_stmts
                        .push(quote! { waymaker_partial::Merge::clear(&mut self.#field_name); });
                } else {
                    apply_field_stmts.push(quote! { waymaker_partial::Apply::apply(&mut self.#field_name, partial.#field_name); });
                    merge_field_stmts.push(quote! { waymaker_partial::Merge::merge(&mut self.#field_name, other.#field_name); });
                    clear_field_stmts
                        .push(quote! { waymaker_partial::Merge::clear(&mut self.#field_name); });
                }
                ...
                if let Some(field_ident) = &field.ident {
                    ...
                    if is_flattened {
                        ...
                        set_field_arms.push(quote! {
                            #field_name_str #(| #field_aliases)* => {
                                if tail.is_empty() {
                                    return Err(waymaker_partial::PartialSetError::EarlyEnd(head.clone()));
                                }
                                waymaker_partial::Set::set(#set_target, tail, val)
                            }
                        });
                    }
                }
                ...
                if let Some(field_ident) = &field.ident {
                    ...
                    let set_logic = if let Some(custom_func) = custom_deserializer {
                        ...
                        quote! {
                            let mut deserializer = waymaker_partial::SimpleDeserializer::from_slice(val);
                        ...
                        quote! {
                            let deserialized = waymaker_partial::deserialize::<#inner_ty>(val)?;
                            #assignment
                        }
                    };
    ...
    let path_setter_impl = if generate_path_setter {
        quote! {
            impl #p_impl_generics waymaker_partial::Set for #partial_name #p_ty_generics #p_where_clause {
                fn set(&mut self, path: &[String], val: &[String]) -> Result<(), waymaker_partial::PartialSetError> {
                    let (head, tail) = path.split_first().ok_or_else(|| {
                        waymaker_partial::PartialSetError::EarlyEnd("root".to_string())
                    })?;
                    ...
                    match head.as_str() {
                        ...
                        _ => {
                            #(
                                match waymaker_partial::Set::set(#flattened_field_targets, path, val) {
                                    Err(waymaker_partial::PartialSetError::Missing(_)) => {}
                                    x => return x,
                                }
                            )*
                            Err(waymaker_partial::PartialSetError::Missing(head.clone()))
                        }
                    }
                }
            }
    ...
    let merge_impl = if enable_merge {
        quote! {
            impl #p_impl_generics waymaker_partial::Merge for #partial_name #p_ty_generics #p_where_clause {
    ...
    let expanded = quote! {
        ...
        impl #impl_generics waymaker_partial::Apply for #name #ty_generics #where_clause {
